...
export default class Animator {
	...
	private _endCallback?: (name: string, state: number) => void;
	...
	constructor(el: HTMLElement, config: AgentWrapper, sounds: Array<string>) {
		...
		for (let i = 1; i < this._data.overlayCount; i++) {
			...
			const inner = this._setupElement(divEl);
			curr.append(inner);
	...
	showAnimation(
		...
		stateChangeCallback: (name: string, state: number) => void,
	) {
		this._exiting = false;
		...
		if (this._endCallback && frameChanged && this._atLastFrame()) {
			if (this._currentAnimation.useExitBranching && !this._exiting) {
				this._endCallback(
					...
					Animator.States.WAITING,
				);
			} else {
				this._endCallback(
					this.currentAnimationName ?? "",
					Animator.States.EXITED,
				);
			}
		}
	}
